...
def html_filter(obj, items):
    ...
    if jetyp(obj, 'Header'):
        ...
        if level == 1:
            ...
            #Odstranime atributy body a popis (nechame teda prazdne atributy v nadpise)
            obj['c'] = [obj['c'][0] + 1, ("Nadpis", [], []), obj['c'][2]]

            items.append(obj)
            if ("body" in options or 'cas' in options):
                items.append(vyrob("Para", [vyrob("Emph", [vyrob("Str", u"Počet bodov: " + options['body'] + u", časový limit: " + options['cas'] + "ms")])]))
            return True
...
def latex_filter(obj, items):
    ...
    if jetyp(obj, 'Header'):
        ...
        if level != 1:
            ...
            vystup += tex(r'\pocetbodov{' + options['body'] + r'}')
            vystup += tex(r'\casovylimit{' + options['cas'] + r'}')
            if 'tcko' in options:
